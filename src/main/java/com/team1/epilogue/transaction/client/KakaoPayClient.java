...
public class KakaoPayClient {
  ...
  private final String CANCEL_URL = ""; // 카카오페이 취소시 redirect 할 URl
  private final String FAIL_URL = ""; // 카카오페이 실패시 redirect 할 URl
  ...
  public KakaoPayResponse prepareCharge(String memberId, int amount) {
    KakaoPayResponse response = webClient.post() // webClient 를 이용해 POST 요청
        ...
        .bodyValue(KakaoPayRequest.makeRequest( // KakaoPayRequest 의 static 메서드를 이용해 요청생성
            kakaoPayCid,
            memberId,
            amount,
            APPROVAL_URL,
  ...
  /**
   * 카카오페이 요청 승인을 받는 메서드입니다.
   * (준비 -> 승인 2단계에 걸쳐 작업이 진행됨)
